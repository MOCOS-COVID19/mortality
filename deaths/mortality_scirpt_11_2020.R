...
deaths_pol <- read_xlsx("statystyka_zgonow.xlsx")
colnames(deaths_pol) <- c("week", "deaths")
deaths_pol$year <- 2020
deaths_pol$country <- "Poland"
deaths_pol$is2020 <- TRUE
deaths_pol <- deaths_pol[-45,]

deaths <- filter(deaths, country!="Poland" | !is2020)
deaths <- rbind(deaths, deaths_pol)
