...
ui <- fluidPage(
    ...
    fluidRow(
        column(4, 
               ...
               selectInput("target", "Zmienna celu:", c("Zgon", "Gorączka", "Kaszel", "Duszność"), "ranger"),
...
server <- function(input, output) {
    ...
    train_model <- reactive({
        ...
        model_ex <- switch (input$model,
               ranger = {
                   model <- ranger(y = y, x = x, probability = TRUE, classification = TRUE)   
                   explain(model, data = x, y = y)
               },
               gbm = {
                   model <- gbm(as.formula(formul), data = data.frame(y = y, x),
                                var.monotone = as.numeric(colnames(x) == "Wiek"))   
                   explain(model, data = x, y = y)
               },
               xgboost = {
                   dat <- as.matrix(createDummyFeatures(x))
                   model <- xgboost(dat,  y, nrounds = 10,
                                    monotone_constraints = as.numeric(colnames(dat) == "Wiek"))
                   explain(model, data = x, y = y,
